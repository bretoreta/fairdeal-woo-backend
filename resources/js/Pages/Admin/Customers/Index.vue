...
const submit = () => {
    form.put(route('admin.customers.update', form.username), {
        onSuccess: () => {
...
<template>
    <AdminLayout title="Customers" page-header="Customers" page-sub-header="Manage customers from here.">
        <div class="grid grid-cols-12 gap-6 mt-5">
            <div class="flex flex-wrap items-center col-span-12 mt-2 sm:flex-nowrap" data-aos="fade-up" data-aos-delay="100" data-aos-once="true">
                <Link :href="route('admin.customers.create')" class="inline-flex items-center justify-center whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 rounded-md px-4">
                    Add New Customer 
                </Link>
            ...
            <div class="col-span-12 overflow-x-auto" data-aos="fade-up" data-aos-delay="200" data-aos-once="true">
                    ...
                    <tbody>
                        <template v-for="customer in customers.data" :key="customer.id">
                            <tr class="bg-white dark:bg-muted/40 dark:text-gray-300">
                                <td class="p-4 first:rounded-l-md last:rounded-r-md shadow-[20px_3px_20px_#0000000b]">
                                    <div class="flex items-center gap-x-3">
                                        <img :src="customer.profile_photo_url" alt="Profile Photo" class="rounded-full object-cover" width="40" height="40">
                                        <Link :href="route('admin.customers.show', customer.username)" class="flex flex-col underline-effect hover:text-blue-500 duration-200">
                                ...
                                <td class="p-4 first:rounded-l-md last:rounded-r-md w-20 border-b-0 py-0 relative">
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                            <DropdownMenuLabel>Manage {{ customer.name }}</DropdownMenuLabel>
                                            <DropdownMenuSeparator />
                                            <DropdownMenuItem>
                                                <button @click="editCustomer(customer)" class="w-full text-left">
                                                    Edit 
                                                </button>
                                            </DropdownMenuItem>
                                            <DropdownMenuItem>
                                                <button @click="confirmDeleteCustomer(customer.username)" class="text-red-500 w-full text-left">
                                                    Delete 
                                                </button>
                                            </DropdownMenuItem>
                                        </DropdownMenuContent>
                ...
                <div v-else class="w-full bg-white dark:bg-muted/40 rounded-md">
                    <EmptyState title="No Customers Added" subtitle="Your application has no customers yet">
                        <template #action>
                            <Button @click="router.visit(route('admin.customers.create'))">Create Customer</Button>
                        </template>
                    </EmptyState>
                </div>
            </div>
            ...
            <template #content>
                <form @submit.prevent="submit">
                    ...
                    <div class="flex items-center gap-4 justify-start mt-6">
                        ...
                        <Link :href="route('admin.customers.index')" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2" variant="outline">
                            Go Back
                        </Link>
                    </div>
                </form>
        ...
        <ConfirmationModal :show="showDeleteModal" @close="showDeleteModal = false">
            ...
            <template #footer>
                <div class="flex items-center gap-3">
                    <Button variant="destructive" @click="deleteCustomer" :class="{ 'opacity-75': isDeletingCustomer }" :disabled="isDeletingCustomer" :loading="isDeletingCustomer">
                        Delete Customer
                    </Button>
